...
# create /tmp/log/uninstall.log & /Library/Logs/Scripts/uninstall/logdate/logtime.log
touch "$logfile"
touch "$tmpfile"
...
if [ -e "$tmpfile" ] ; then
    ...
    while read -r line ; do
        ...
        line=`python -c "print(__import__('re').sub(r'(\x1b\[[0-9][0-9;]*m)|(\^D\x08\x08)', '', __import__('sys').stdin.readline().strip(), flags=__import__('re').IGNORECASE))" <<< $line`
...
if ( $interrupred ) ; then
    echo "ERR: $uninstall procedure interrupred" >> "$logfile"
fi
