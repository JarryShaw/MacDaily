    ...
    for name in $arg_pkg ; do
        ...
        case $name in
            all)
                ...
                if ( $arg_q ) ; then
                    $logprefix softwareupdate --install --no-scan --all $restart $verbose $quiet > /dev/null 2>&1
                else
                    $logprefix softwareupdate --install --no-scan --all $restart $verbose $quiet
                fi
            ...
            *)
                ...
                if [[ ! -z $flag ]] ; then
                    ...
                    if ( $arg_q ) ; then
                        $logprefix softwareupdate --install --no-scan $name $restart $verbose $quiet > /dev/null 2>&1
                    else
                        $logprefix softwareupdate --install --no-scan $name $restart $verbose $quiet
                    fi
                    $logprefix echo | $logsuffix
                else
