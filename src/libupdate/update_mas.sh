    ...
    for name in $arg_pkg ; do
        ...
        case $name in
            all)
                ...
                if ( $arg_q ) ; then
                    $logprefix mas upgrade $verbose $quiet > /dev/null 2>&1
                else
                    $logprefix mas upgrade $verbose $quiet
                fi
            ...
            *)
                ...
                if [[ ! -z $flag ]] ; then
                    ...
                    if ( $arg_q ) ; then
                        $logprefix mas upgrade $number $verbose $quiet > /dev/null 2>&1
                    else
                        $logprefix mas upgrade $number $verbose $quiet
                    fi
                    $logprefix echo | $logsuffix
                else
