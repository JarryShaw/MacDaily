...
class CaskLogging(CaskCommand, LoggingCommand):
    ...
    def _check_exec(self):
        try:
            ...
            text = (f'macdaily-{self.cmd}: {red}cask{reset}: you may find Bundler on '
    ...
    def _proc_logging(self, path):
        ...
        with open(logfile, 'w') as file:
            file.writelines(filter(lambda s: s.startswith('cask'), context.strip().splitlines(True)))  # pylint: disable=filter-builtin-not-iterating
