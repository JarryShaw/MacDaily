...
function piplogging {
    ...
    if [ -e $prefix/$suffix ] ; then
        ...
        for name in $arg_pkg ; do
            case $name in
                all)
                    echo -e "++ pip$pprint list --no-cache-dir --format freeze | grep \"==\" | sed \"s/\(.*\)*==.*/\1/\"" >> "$tmpfile"
                    $logprefix $prefix/$suffix -m pip list --no-cache-dir --format freeze 2>/dev/null | grep "==" | sed "s/\(.*\)*==.*/\1/"
                    echo >> "$tmpfile" ;;
                *)
